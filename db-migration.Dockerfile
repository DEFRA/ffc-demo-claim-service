...
CMD ["sh", "-c", "az login --service-principal --username $AZURE_CLIENT_ID --federated-token $(cat $AZURE_FEDERATED_TOKEN_FILE) --tenant $AZURE_TENANT_ID; ACCESS_TOKEN=$(az account get-access-token --resource-type oss-rdbms --query 'accessToken' --output tsv); export ACCESS_TOKEN; cd scripts/migration; sh database-up $ACCESS_TOKEN;"]
